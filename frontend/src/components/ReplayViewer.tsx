...
export default function ReplayViewer({ taskId }: { taskId: string }) {
  ...
  useEffect(() => {
    ...
    setError(null);
    setActiveIndex(0);
    api.getReplay(taskId).then((response) => {
  ...
  if (!replay) return <p className="rounded-lg border border-stone-200 p-5 text-sm text-stone-500">Loading replay data…</p>;
  if (!replay.replay_steps.length) return <p className="rounded-lg border border-dashed border-stone-300 p-5 text-sm text-stone-500">No replay steps are available for this decision.</p>;
  ...
  return (
    <section aria-label="Decision replay">
      <p className="mb-5 text-sm text-stone-600">Reconstruct this decision from its stored audit record. No agent is re-executed.</p>
      <div className="mb-5 flex items-center justify-between text-sm">
        <span className="font-medium text-stone-800">Step {activeIndex + 1} of {replay.replay_steps.length}</span>
        <time className="text-stone-400">{formatTimestamp(step.timestamp)}</time>
      </div>
      <div className="h-1.5 overflow-hidden rounded-full bg-stone-200" aria-label={`${Math.round(progress)} percent through replay`}>
      ...
      <div className="mt-5 flex justify-between gap-3">
        <button type="button" disabled={activeIndex === 0} onClick={() => setActiveIndex((index) => index - 1)} className="rounded-md border border-stone-300 px-3 py-2 text-sm font-medium text-stone-700 disabled:cursor-not-allowed disabled:opacity-40">← Previous</button>
        <button type="button" disabled={activeIndex === replay.replay_steps.length - 1} onClick={() => setActiveIndex((index) => index + 1)} className="rounded-md bg-stone-900 px-3 py-2 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-40">Next →</button>
      </div>
      {replay.final_decision && (
        <section className="mt-6 border-t border-stone-200 pt-5">
          <h3 className="text-sm font-semibold uppercase tracking-wider text-stone-500">Final Decision</h3>
          <p className="mt-2 font-medium capitalize text-stone-900">{replay.final_decision.outcome.replace(/_/g, " ")}</p>
